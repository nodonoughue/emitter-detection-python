...
def example1(rng=np.random.default_rng()):
    ...
    num_mc_trials = int(2)  # TODO: Increase to 1000
    num_iterations = int(10)
    alpha = .3
    beta = .8
    epsilon = int(1000)  # desired search resolution size
    ...
    plt.scatter(x_chan_ho[0, 0] / 1e3, x_chan_ho[1, 0] / 1e3, '*', label='Chan-Ho')
    plt.scatter(x_bf[0, 0] / 1e3, x_bf[1, 0] / 1e3, 'o', label='BestFix')

    plt.plot(crlb_ellipse[0, :] / 1e3, crlb_ellipse[1, :] / 1e3, 'k', linewidth=.5, label='90% Error Ellipse')

    wd = 1.4 * np.max(np.abs(crlb_ellipse[0, :] - x_source[0]))
    ht = wd * 7 / 8
    plt.ylim([-1, 1] * ht / 1e3 + x_source(2) / 1e3)
    plt.xlim([-1, 1] * wd / 1e3 + x_source(1) / 1e3)
    plt.xlabel('Cross-range [km]')
    plt.ylabel('Down-range [km]')
    ...
    cep50_chan_ho = utils.errors.compute_cep50(cov_chan_ho)
