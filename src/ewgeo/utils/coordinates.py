...
from .constants import first_ecc_sq, radius_earth_true, second_ecc_sq, semimajor_axis_km, semiminor_axis_km
...
def aer_to_enu(az, el, rng, angle_units='deg'):
    ...
    az_rad = convert(az, from_unit=angle_units, to_unit='rad')
    el_rad = convert(el, from_unit=angle_units, to_unit='rad')
...
def ecef_to_enu_vel(vel_x, vel_y, vel_z, lat_ref, lon_ref, angle_units='deg'):
    ...
    # Compute some sin/cos terms; we need lat_ref/lon_ref in radians first
    lat_ref_rad = convert(lat_ref, from_unit=angle_units, to_unit='rad')
    lon_ref_rad = convert(lon_ref, from_unit=angle_units, to_unit='rad')
...
def ecef_to_lla(x, y, z, angle_units='deg', dist_units='m'):
    ...
    lon = convert(lon_rad, from_unit="rad", to_unit=angle_units)
    alt = convert(alt_m, from_unit="m", to_unit=dist_units)
...
def enu_to_aer(east, north, up, angle_units='deg'):
    ...
    az = convert(az_rad, from_unit="rad", to_unit=angle_units)
    el = convert(el_rad, from_unit="rad", to_unit=angle_units)
...
def enu_to_ecef(east, north, up, lat_ref, lon_ref, alt_ref, angle_units='deg', dist_units='m'):
    ...
    # lat_rad = convert(lat_ref, from_unit=angle_units, to_unit='rad')
    # lon_rad = convert(lon_ref, from_unit=angle_units, to_unit='rad')
    #
...
def lla_to_ecef(lat, lon, alt, angle_units='deg', dist_units='m'):
    ...
    semi_major_axis_m = semimajor_axis_km*1e3
    ecc_sq = first_ecc_sq

    # Make sure Lat/Lon are in rad, alt in meters
    lat_rad = convert(lat, from_unit=angle_units, to_unit='rad')
    lon_rad = convert(lon, from_unit=angle_units, to_unit='rad')
    alt_m = convert(alt, from_unit=dist_units, to_unit='m')
