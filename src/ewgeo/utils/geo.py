...
from . import is_broadcastable, safe_2d_shape
...
def calc_range(x1, x2):
    ...
    sz_1 = safe_2d_shape(x1)
    sz_2 = safe_2d_shape(x2)
    if sz_1[0] != sz_2[0]:
...
def calc_doppler(x1, v1, x2, v2, f):
    ...
    # Reshape inputs
    num_dims, num_sources = safe_2d_shape(x1)
    _, num_sources2 = safe_2d_shape(v1)
    num_dims2, num_sensors = safe_2d_shape(x2)
    _, num_sensors2 = safe_2d_shape(v2)

    if num_dims != num_dims2 or \
            (not is_broadcastable(x1, v1)) or \
            (not is_broadcastable(x2, v2)):
