...
def log_likelihood_uncertainty(x_sensor, zeta, cov: CovarianceMatrix, cov_pos: CovarianceMatrix, theta, ref_idx=None,
    ...
    # parameter vector represented by x_source. Instead of nDim x nSourcePos, the matrix is assumed to have
    # size (nDim + nTDOA + nDim*nTDOA) x nSourcePos, where the first nDim rows represent the unknown target
    # position, the next nTDOA rows are measurement biases, and the remainder are sensor positions.
    do_pos_error = cov_pos is not None
    parameter_indices = utils.make_uncertainty_indices(num_dim=n_dim, num_tdoa=n_sensor,
                                                       do_tdoa_bias=do_sensor_bias,
                                                       do_tdoa_pos_error=do_pos_error)
    beta = x_sensor.ravel()  # Assume the x_sensor positions provided are truth.
